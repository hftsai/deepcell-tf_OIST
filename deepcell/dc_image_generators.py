...
"""
from deepcell import reshape_matrix, reshape_targets
...
class ImageSampleArrayIterator(Iterator):
    def __init__(self, train_dict, image_data_generator,
        ...
        if train_dict["labels"] is not None and len(train_dict["pixels_x"]) != len(train_dict["labels"]):
            raise Exception('Number of sampled pixels and y (labels) '
                            ...
                            'Found: Number of sampled pixels = %s, y.shape = %s' % (
                            len(train_dict["pixels_x"]), np.asarray(train_dict["labels"]).shape))
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        if type(index_array) == tuple:
            batch_length = len(index_array[0])
            indexes = index_array[0]
        elif type(index_array) == np.ndarray:
            batch_length = index_array.shape[0]
            indexes = list(index_array)
...
class SampleDataGenerator(ImageDataGenerator):
    def sample_flow(self, train_dict, batch_size=32, shuffle=True, seed=None,
                    save_to_dir=None, save_prefix='', save_format='png', data_format=None):
...
class ImageFullyConvIterator(Iterator):
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        #index_array = index_array[0]
        #if type(index_array) == list:
        #    batch_len = len(index_array)
        #elif type(index_array) == np.ndarray:
        #    batch_len = index_array.shape[0]
        #batch_len = index_array[0]
        batch_len = len(index_array)
        
        if self.channels_axis == 1:
            batch_x = np.zeros(tuple([batch_len] + [self.x.shape[1], self.x.shape[2], self.x.shape[3]]))
            if self.y is not None:
                batch_y = np.zeros(tuple([batch_len] + [self.y.shape[1], self.target_size, self.target_size]))
        else:
            batch_x = np.zeros(tuple([batch_len] + [self.x.shape[2], self.x.shape[3]] + [self.x.shape[1]]))
            if self.y is not None:
                batch_y = np.zeros(tuple([batch_len] + [self.target_size, self.target_size] + [self.y.shape[1]]))
        ...
        for i, j in enumerate(index_array):
            ...
            if self.y is not None:
                ...
                if self.target_size is not None:
                    y = reshape_targets(y, self.target_size)
            else:
                x = self.image_data_generator.random_transform(x.astype(K.floatx()))
...
class ImageFullyConvGatherIterator(Iterator):
    def __init__(self, train_dict, image_data_generator,
        ...
        expected_label_size = (self.x.shape[0], train_dict["labels"].shape[1], self.x.shape[2] - 2 * self.win_x,
                               self.x.shape[3] - 2 * self.win_y)
        if train_dict["labels"] is not None and train_dict["labels"].shape != expected_label_size:
            raise Exception('The expected conv-net output and label image'
                            'should have the same size. '
                            'Found: expected conv-net output shape = %s, label image shape = %s' % expected_label_size,
...
class MovieArrayIterator(Iterator):
    def __init__(self, train_dict, movie_data_generator,
        ...
        if train_dict["labels"] is not None and train_dict["channels"].shape[0] != train_dict["labels"].shape[0]:
            raise Exception('Data movie and label movie should have the same size'
                            'Found data movie size = %s and and label movie size = %s' % (
                            train_dict["channels"].shape, train_dict["labels"].shape)
                            )
        if data_format is None:
            data_format = K.image_dim_ordering()
        self.x = np.asarray(train_dict["channels"], dtype=K.floatx())
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        if self.channels_axis == 1:
            batch_x = np.zeros(
                tuple([len(index_array), self.x.shape[1], self.number_of_frames] + list(self.x.shape[3:])))
            if self.y is not None:
                batch_y = np.zeros(
                    tuple([len(index_array), self.y.shape[1], self.number_of_frames] + list(self.y.shape[3:])))
...
class BoundingBoxIterator(Iterator):
    ...
    def anchor_targets(
            ...
            **kwargs):
        return anchor_targets_bbox(image_shape, annotations, num_classes, mask_shape, negative_overlap,
                                   positive_overlap, **kwargs)
