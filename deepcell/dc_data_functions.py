...
def sample_label_matrix(feature_mask, edge_feature, window_size_x=30, window_size_y=30, sample_mode="subsample",
                        border_mode="valid", output_mode="sample"):
    # Create a list of the maximum pixels to sample from each feature in each data set. If sample_mode is "subsample",
                    ...
                    for i in rand_ind:
                        if pixel_counter < max_num_of_pixels:
                            if border_mode == "same":
                                ...
                                feature_rows_temp[i] + window_size_x < image_size_x)
                                             and (feature_cols_temp[i] - window_size_y > 0) and (
...
def reshape_targets(targets, shape):
    new_targets = np.zeros((targets.shape[0], shape, shape))

    for i in xrange(targets.shape[0]):
        new_targets[i] = scipy.misc.imresize(targets[i], (shape, shape), interp='nearest')
    new_targets[np.where(new_targets > 0)] = 1
    return new_targets
        ...
        for i in xrange(rep_number):
            for j in xrange(rep_number):
                if i != rep_number - 1 and j != rep_number - 1:
                    new_channels[counter, :, :, :] = channels[batch, :, i * reshaped_size:(i + 1) * reshaped_size,
                                                     j * reshaped_size:(j + 1) * reshaped_size]
                    new_feature_mask[counter, :, :, :] = feature_mask[batch, :,
                                                         i * reshaped_size:(i + 1) * reshaped_size,
                                                         j * reshaped_size:(j + 1) * reshaped_size]
                if i == rep_number - 1 and j != rep_number - 1:
                    new_channels[counter, :, :, :] = channels[batch, :, -reshaped_size:,
                                                     j * reshaped_size:(j + 1) * reshaped_size]
                    new_feature_mask[counter, :, :, :] = feature_mask[batch, :, -reshaped_size:,
                                                         j * reshaped_size:(j + 1) * reshaped_size]
                if i != rep_number - 1 and j == rep_number - 1:
                    new_channels[counter, :, :, :] = channels[batch, :, i * reshaped_size:(i + 1) * reshaped_size,
                                                     -reshaped_size:]
                    new_feature_mask[counter, :, :, :] = feature_mask[batch, :,
                                                         i * reshaped_size:(i + 1) * reshaped_size, -reshaped_size:]
                if i == rep_number - 1 and j == rep_number - 1:
...
def reshape_movie(channels, feature_mask, reshaped_size=256):
    ...
    new_channels = np.zeros((new_batch_size, channels.shape[1], channels.shape[2], reshaped_size, reshaped_size),
                            dtype=K.floatx())
    new_feature_mask = np.zeros((new_batch_size, feature_mask.shape[1], reshaped_size, reshaped_size), dtype="int32")
        ...
        for i in xrange(rep_number):
            for j in xrange(rep_number):
                if i != rep_number - 1 and j != rep_number - 1:
                    new_channels[counter, :, :, :, :] = channels[batch, :, :, i * reshaped_size:(i + 1) * reshaped_size,
                                                        j * reshaped_size:(j + 1) * reshaped_size]
                    feature_mask_temp = relabel_movie(feature_mask[batch, :, i * reshaped_size:(i + 1) * reshaped_size,
                                                      j * reshaped_size:(j + 1) * reshaped_size])
                    new_feature_mask[counter, :, :, :] = feature_mask_temp
                if i == rep_number - 1 and j != rep_number - 1:
                    new_channels[counter, :, :, :, :] = channels[batch, :, :, -reshaped_size:,
                                                        j * reshaped_size:(j + 1) * reshaped_size]
                    feature_mask_temp = relabel_movie(
                        feature_mask[batch, :, -reshaped_size:, j * reshaped_size:(j + 1) * reshaped_size])
                    new_feature_mask[counter, :, :, :] = feature_mask_temp
                if i != rep_number - 1 and j == rep_number - 1:
                    new_channels[counter, :, :, :, :] = channels[batch, :, :, i * reshaped_size:(i + 1) * reshaped_size,
                                                        -reshaped_size:]
                    feature_mask_temp = relabel_movie(
                        feature_mask[batch, :, i * reshaped_size:(i + 1) * reshaped_size, -reshaped_size:])
                    new_feature_mask[counter, :, :, :] = feature_mask_temp
                if i == rep_number - 1 and j == rep_number - 1:
                    new_channels[counter, :, :, :, :] = channels[batch, :, :, -reshaped_size:, -reshaped_size:]
...
def make_training_data(max_training_examples=1e7, window_size_x=30, window_size_y=30,
                       ...
                       file_name_save=os.path.join("/home/vanvalen/DeepCell/training_data_npz/RAW40X_tube/",
                                                   "RAW_40X_tube_61x61.npz"),
    ...
    print '[MAKE DATA] channels', channels.shape
    print '[MAKE DATA] feature_mask', feature_mask.shape
        ...
        for channel_counter, channel in enumerate(channel_names):
            for img in imglist:
                if fnmatch.fnmatch(img, r'*' + channel + r'*'):
                    ...
                    if process:
                        channel_img = process_image(channel_img, window_size_x, window_size_y, std=process_std,
    ...
    # print '[MAKE DATA] before reshape, channels', channels.shape
    # print '[MAKE DATA] before reshape, feature_mask', feature_mask.shape
    if reshape_size is not None:
        channels, feature_mask = reshape_matrix(channels, feature_mask, reshaped_size=reshape_size)
    # print '[MAKE DATA] after reshape, channels', channels.shape
    # print '[MAKE DATA] after reshape, feature_mask', feature_mask.shape
    ...
    if output_mode == "sample":
        # print '[MAKE DATA], output mode: sample'
        feature_rows, feature_cols, feature_batch, feature_label = sample_label_matrix(feature_mask, edge_feature,
    ...
    if output_mode == "conv":
        # print '[MAKE DATA], output mode: conv'
        ...
        # Save training data in npz format
        np.savez(file_name_save, class_weights=weights, channels=channels, y=feature_mask, y_sample=feature_mask_sample,
                 win_x=window_size_x, win_y=window_size_y)

    if output_mode == "bbox":
        print '[MAKE DATA], output mode: bbox'
    ...
    if output_mode == "disc":
        print '[MAKE DATA], output mode: disc'
        ...
        feature_rows, feature_cols, feature_batch, feature_label = sample_label_matrix(feature_mask, edge_feature,
                                                                                       ...
                                                                                       window_size_x=window_size_x,
                                                                                       window_size_y=window_size_y)
        print '[MAKE DATA] feat rows, cols, batch, label', feature_rows, feature_cols, feature_batch, feature_label
        print '> feat rows', np.min(feature_rows), np.max(feature_rows)
        print '> feat cols', np.min(feature_cols), np.max(feature_cols)
        print '> feat batch', np.min(feature_batch), np.max(feature_batch)
        print '> feat label', np.min(feature_label), np.max(feature_label)

        # Compute weights for each class
        weights = class_weight.compute_class_weight('balanced', classes=np.unique(feature_label), y=feature_label)

        # Create mask with labeled cells
        feature_mask_label = np.zeros((feature_mask.shape[0], 1, feature_mask.shape[2], feature_mask.shape[3]),
                                      dtype='int32')
        for b in xrange(feature_mask.shape[0]):
            interior_mask = feature_mask[b, 1, :, :]
            feature_mask_label[b, :, :, :] = label(interior_mask)

        print '[MAKE DATA] feature_mask_label, channels', channels.shape
        print '[MAKE DATA] feature_mask_label, feature_mask', feature_mask.shape

        max_cells = np.amax(feature_mask_label)
        feature_mask_binary = np.zeros(
            (feature_mask.shape[0], max_cells + 1, feature_mask.shape[2], feature_mask.shape[3]), dtype='int32')
        for b in xrange(feature_mask.shape[0]):
            label_mask = feature_mask_label[b, :, :, :]
            for l in xrange(max_cells + 1):
                feature_mask_binary[b, l, :, :] = label_mask == l

        print '[MAKE DATA] feature_mask_binary, channels', channels.shape
        print '[MAKE DATA] feature_mask_binary, feature_mask', feature_mask.shape

        if border_mode == "valid":
            feature_mask_label_trimmed = feature_mask_label[:, :, window_size_x:-window_size_x,
                                         window_size_y:-window_size_y]
            feature_mask_label = feature_mask_label_trimmed

            feature_mask_binary_trimmed = feature_mask_binary[:, :, window_size_x:-window_size_x,
                                          window_size_y:-window_size_y]
            feature_mask_binary = feature_mask_binary_trimmed

        # Save training data in npz format
        np.savez(file_name_save, class_weights=weights, channels=channels, y=feature_mask_binary, win_x=window_size_x,
                 win_y=window_size_y)
    ...
    if verbose:
        ...
        for j in xrange(feature_mask.shape[0]):
            print 1.0 / 3.0 * np.sum(feature_mask[j, :, :, :].astype(K.floatx()), axis=(0, 1, 2)) / np.sum(
                feature_mask[j, :, :, :].astype(K.floatx()), axis=(1, 2))
...
def make_training_data_movie(window_size_x=30, window_size_y=30,
                             ...
                             file_name_save=os.path.join('/home/vanvalen/DeepCell/training_data_npz/HeLa_movie/',
                                                         'HeLa_movie_61x61.npz'),
    ...
    # Load one file to get image sizes
    image_size_x, image_size_y = get_image_sizes(os.path.join(direc_name, training_direcs[0], raw_image_direc),
                                                 channel_names)
    ...
    if output_mode == "disc":
        ...
        feature_mask_binary = np.zeros((feature_label.shape[0], max_cells + 1, feature_label.shape[1],
                                        feature_label.shape[2], feature_label.shape[3]), dtype='int32')
